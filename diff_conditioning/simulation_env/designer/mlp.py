...
class MLP(Base):
    '''
    Use an MLP to predict the geometry, softness and actuator of a robot, no actuation matrix
    '''
    def __init__(
        self,
        env:BaseEnv, coord_input_names:List[str],
        filters:List[int], # The number of hidden layers and its dimension
        n_actuators:int, activation:str,
        lr:float,
        geometry_offset,
        softness_offset,
        seed_meshes=[],
