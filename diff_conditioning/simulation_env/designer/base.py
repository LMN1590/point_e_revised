...
class Base(nn.Module):
    ...
    def update(self,grad,retain_graph=False,it=0):
        self.optim.zero_grad()
        # with open(f'./local/gripping/debug_update_{it}.txt','w')as f:
        #     print(f'{self.out_cache}\n\n\n\n{grad}',file=f)
    ...
    # region PCD Operation
    def save_pcd(self, filepath, design, mode='geometry'):
        pcd = self.design_to_pcd(design, mode)
        o3d.io.write_point_cloud(filepath, pcd)
        return pcd

    def design_to_pcd(self, design:Dict[str,torch.Tensor], mode):
        ...
        assert np.allclose(mask.astype(float), x_mask) # sanity check
        xyz = xyz[mask]
    ...
    def save_voxel_grid(self, filepath:str, design:Dict[str,torch.Tensor], mode='geometry'):
        geometry = design['geometry'].data.cpu().numpy()
        mask = (geometry > self.env.design_space.cfg.p_rho_lower_bound_mul)
        if mode == 'geometry':
            pcd = o3d.geometry.PointCloud()
