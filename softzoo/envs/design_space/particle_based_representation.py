...
from ...configs.config_dataclass import DesignSpaceConfig

@ti.data_oriented
class ParticleBasedRepresentation(DesignRepresentation):
    def __init__(self, sim: TaichiSim, cfg: DesignSpaceConfig):
    ...
    # region Interface for Implementation
    def initialize(self):
    ...
    def set_design_kernel(self):
        for i in range(self.n_actuators):
            if i < self.buffer['actuator_direction'].shape[0]:
                a = self.buffer['actuator_direction'][i]
                a = ti.math.mod(a,2*ti.math.pi)
                azimuth = a[0]
                inclination = a[1]
                
                direction = ti.Vector([
                    ti.math.cos(azimuth) * ti.math.sin(inclination),
                    ti.math.sin(azimuth) * ti.math.sin(inclination),
                    ti.math.cos(inclination)
                ], dt=F_DTYPE)
                
                self.sim.solver.muscle_direction[i] = direction.outer_product(direction)
            else:
                self.sim.solver.muscle_direction[i] = ti.Matrix.zero(F_DTYPE,self.sim.solver.dim, self.sim.solver.dim) 
        for p in range(self.sim.solver.n_particles[None]):
            ...
            if self.is_robot(id):
                ...
                # update density
                self.sim.solver.fixed[p] = self.is_passive_fixed[p]
