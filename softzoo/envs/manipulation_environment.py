...
class ManipulationEnvironment(BaseEnv):
    def __init__(
        self,
        cfg_file:str, out_dir:str, device:Optional[Literal['numpy','torch_cpu','torch_gpu']]  = 'numpy',
        cfg_kwargs: Optional[Dict] = dict(),ti_device_memory_fraction: Optional[float] = None, initialize: Optional[bool] = False
    ):
    ...
    def initialize(self):
        ...
        if self.cfg.ENVIRONMENT.CUSTOM.has_matter_on_ground:
            for matter_material in self.matter_materials:
                assert matter_material in self.sim.solver.active_materials['members'], f'Matter material {matter_material.name} is not active'
    ...
    def step(self, action,fixed_v):
        obs, reward, done, info = super().step(action,fixed_v)
