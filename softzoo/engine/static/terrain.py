...
from ...configs.config_dataclass import TerrainConfig

if TYPE_CHECKING:
    from ..mpm_solver import MPMSolver

def add(solver:'MPMSolver', cfg:TerrainConfig):
    # print('added flat surface')
    ...
    assert padded_mul % 2 == 1, "Padded Multiplier needs to be an odd number"
    ...
    def generate_surface():
        ...
        if solver.dim == 2:
            ...
            for i in range(padded_mul * resolution):
                ...
                surface_np_padded[i, 1] = noise.pnoise1(
                    non_padded_i/scale,
                    octaves=octaves,
                    persistence=persistence,
                    lacunarity=lacunarity,
            ...
            for i in range(resolution):
                for j in range(resolution):
                    surface_np[i, j, 0] = i
                    surface_np[i, j, 2] = j
                    surface_np[i, j, 1] = noise.pnoise2(
                        i/scale, # height is y-component in mpm
                        j/scale,
                        octaves=octaves,
                        persistence=persistence,
                        lacunarity=lacunarity,
            ...
            for i in range(padded_mul * resolution):
                for j in range(padded_mul * resolution):
                    ...
                    surface_np_padded[i, j, 1] = noise.pnoise2(
                        non_padded_i/scale, # height is y-component in mpm
                        non_padded_j/scale,
                        octaves=octaves,
                        persistence=persistence,
                        lacunarity=lacunarity,
        ...
        if signed_dist < cfg.signed_dist_thresh:
            if ti.static(surface == Surface.Sticky.value):
                ...
                if ti.static(surface == Surface.Slip.value):
                    v = v - cfg.dampen_coeff * n * normal_component
                ...
                grid_v_out_I = ti.min(v,ti.Vector.zero(F_DTYPE, solver.dim))
