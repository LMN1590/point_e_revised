...
from typing import Union,Tuple,List,Any,Optional,Callable,Optional

from ..configs.config_dataclass import SolverConfig
from ..configs.item_configs.particle_group_info import get_cfg_defaults as get_particle_group_info_defaults
from ..utils.computation_utils import compute_lame_parameters
from ..utils.const import I_DTYPE,F_DTYPE
from ..utils.taichi_utils import TiDeviceInterface,clamp_at_zero

from .static import Static
from .primitives import PrimitiveBase,Primitive
...
class MPMSolver:
    def __init__(self,cfg:SolverConfig,robot_id:int):
        ...
        # region Stiffness and Deformation
        ...
        # region Setup Environment Interaction
        ...
        self.primitives:List[PrimitiveBase] = []
        
        self.set_gravity(cfg.gravity)
        # endregion
        
        # region Particle-Variant Var Setup
    ...
    def initialize(self):
        ...
        material_ids = -np.ones((self.n_particles[None]),dtype=int)
        ...
        # region Actuators Configuration
    ...
    def add(self,item: Any,item_cfg: Optional[Any] = None):
    ...
    def get_latest_s(self, consider_cycle=True):
        ...
        if consider_cycle: latest_s = self.get_cyclic_s(latest_s)
        return latest_s
    ...
    def step(
        ...
        while frame_time_left > 0:
            ...
            self.substep(action, dt, fixed_v=fixed_v)
            if not self.needs_grad:
    ...
    @ti.ad.grad_replaced
    def substep(self,action:Any, dt:float,recompute:Optional[bool] = False,fixed_v:List[float]=[0.,0.,0.]):
        s = self.current_s_local
        self.fixed_v[s] = ti.Vector(fixed_v)
    ...
    def substep_grad(
        self,
        action:Any, dt:float, pre_grad_fn:Optional[Callable] = None,
        compute_grad:Optional[bool] = True, #fixed_v:List[float]= [0.,0.,0.]
    ):
    ...
    def p2g(self, s: I_DTYPE, dt: F_DTYPE):
        for p in range(self.n_particles[None]):
            ...
            if p_mass > 0:
                ...
                # Fetch base grid node for this particle
                base = ti.floor(self.x[s, p] * self.inv_dx - 0.5).cast(I_DTYPE)
                fx = self.x[s, p] * self.inv_dx - base.cast(F_DTYPE)
                w = [0.5 * (1.5 - fx)**2, 0.75 - (fx - 1)**2, 0.5 * (fx - 0.5)**2]

                # Skip full stress computation for fixed particles
                if self.fixed[p] >= 1.0:
                    # Maintain F and Jp consistency (already in your code)
                    self.Jp[next_s, p] = self.Jp[s, p]
                    self.F[next_s, p] = self.F[s, p]
                    # self.v[s,p] += self.fixed_v[s]

                    # Inject fixed velocity to the grid as momentum
                    for offset in ti.static(ti.grouped(self.stencil_range())):
                        ...
                        # Use fixed velocity instead of particle's v and affine
                        self.grid_v_in[s, base + offset] += weight * (p_mass * self.v[s,p])
                        self.grid_v_fixed[s,base+offset] = 1.0

                        # OPTIONAL: comment this line to prevent fixed mass from contributing
                        self.grid_m[s, base + offset] += weight * p_mass

                else:
                    # === Normal dynamic particles ===
                    stress = ti.Matrix.zero(F_DTYPE, self.dim, self.dim)
                    for material_id in ti.static(self.active_materials['ids']):
                        if self.material[p] == material_id:
                            stress = self.material_models[material_id]['p2g'](
                                p, s, next_s, dt, p_mass, self.x, self.v, self.F, self.C, self.Jp,
    ...
    def grid_op(self, s: I_DTYPE, dt: F_DTYPE):
        for I in ti.grouped(ti.ndrange(*self.res)):
            ...
            if self.grid_v_fixed[s,I]==1.0:
                v_out = self.fixed_v[s]
    ...
    def g2p(self, s: I_DTYPE, dt: F_DTYPE):
        for p in range(self.n_particles[None]):
            ...
            if p_mass > 0: # NOTE: allow zero-mass particle
                ...
                # if self.fixed[p]>=1.00:
                #     self.v[next_s,p] = self.v[s,p] #+ self.fixed_v[s]
                #     self.C[next_s,p] = self.C[s,p]
                # else:
    ...
    @ti.kernel
    def save_sim_checkpoint_to_ext(
        self, 
        s: I_DTYPE, x: ti.types.ndarray(), v: ti.types.ndarray(), F: ti.types.ndarray(),
    ...
    @ti.kernel
    def load_sim_checkpoint_from_ext(
        self, 
        s: I_DTYPE, x: ti.types.ndarray(), v: ti.types.ndarray(), F: ti.types.ndarray(),
        C: ti.types.ndarray(), Jp: ti.types.ndarray()
    ):
    ...
    @ti.kernel
    def copy_substep(self, source: I_DTYPE, target: I_DTYPE):
        for p in range(self.n_particles[None]):
    ...
    @ti.kernel
    def debug_check_nan(self, s: I_DTYPE,step:I_DTYPE)->I_DTYPE:
        check_nan = False
        prefix = step
        for p in range(self.n_particles[None]):
            if not check_nan:
                if ti.math.isnan(self.U[s, p]).sum() > 0:
                    print(f'{prefix} - Matrix U got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.sig[s, p]).sum() > 0:
                    print(f'{prefix} - Matrix sig got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.V[s, p]).sum() > 0:
                    print(f'{prefix} - Matrix V got nan at timestep {s}, particle {p}')
                    check_nan = True

                if ti.math.isnan(self.x[s, p]).sum() > 0:
                    print(f'{prefix} - Position got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.v[s, p]).sum() > 0:
                    print(f'{prefix} - Velocity got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.F_tmp[s, p]).sum() > 0:
                    print(f'{prefix} - Deformable Matrix F_tmp got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.F[s, p]).sum() > 0:
                    print(f'{prefix} - Deformable Matrix F got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.C[s, p]).sum() > 0:
                    print(f'{prefix} - Affine Matrix C got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.Jp[s, p]) > 0:
                    print(f'{prefix} - Matrix Jp got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.act_buffer[s, p]) > 0:
                    print(f'{prefix} - Act buffer got nan at timestep {s}, particle {p}')
                    check_nan = True
                if ti.math.isnan(self.actuation[s, p]) > 0:
                    print(f'{prefix} - Actuation got nan at timestep {s}, particle {p}')
                    check_nan = True

        for I in ti.grouped(ti.ndrange(*self.res)):
            if not check_nan:
                if ti.math.isnan(self.grid_v_in[s,I]).sum() > 0:
                    print(f'{prefix} - grid v in at {s} - {I} got nan')
                    check_nan = True
                if ti.math.isnan(self.grid_v_out[s,I]).sum() > 0:
                    print(f'{prefix} - grid v out at {s} - {I} got nan')
                    check_nan = True
                if ti.math.isnan(self.grid_m[s,I]) > 0:
                    print(f'{prefix} - grid m at {s} - {I} got nan')
                    check_nan = True
        if check_nan:
            print('Got nan at timestep ', s)
        return ti.cast(check_nan,dtype=I_DTYPE)
