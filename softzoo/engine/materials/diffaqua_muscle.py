...
def get_material_model(solver: 'MPMSolver'):
    dim = solver.dim
    dx = solver.dx
    ...
    if use_actuator_to_specify_muscle_direction:
        ...
        solver.muscle_direction = ti.Matrix.field(solver.dim, solver.dim, dtype=F_DTYPE, shape=(n_actuators,),needs_grad=solver.needs_grad)
        ...
        solver.muscle_direction = ti.Matrix.field(solver.dim, solver.dim, dtype=F_DTYPE,needs_grad=solver.needs_grad)
    ...
    def p2g_corotated(
        ...
        # Get Lame parameters
        h = ti.cast(DIFF_AQUA_H, F_DTYPE) # hardening coefficient
        mu = mu_[p] * h
        lambd = lambd_[p] * h
