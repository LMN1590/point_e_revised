...
class Box(PrimitiveBase):
    def __init__(self, solver:'MPMSolver', cfg:BoxConfig):
        ...
        if not self.is_rigid:
            ...
            assert self.solver.n_particles[None] + num_new_particles <= self.solver.max_num_particles, \
    ...
    def seed(self, s: I_DTYPE, num_new_particles: I_DTYPE, material: I_DTYPE, particle_id: I_DTYPE):
        for p in range(self.solver.n_particles[None],self.solver.n_particles[None] + num_new_particles):
            ...
            self.solver.seed_nonoverlap_particle(
                s, x, v, material, particle_id, 
                self.particle_info.p_rho_0,self.particle_info.mu_0, self.particle_info.lambd_0
            )
