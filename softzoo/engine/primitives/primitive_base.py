...
from ...configs.config_dataclass import (
    BoxConfig,SphereConfig,MeshConfig,EllipsoidConfig,
    PrimitiveBaseConfig
)
from ...configs.item_configs.primitives import get_cfg_defaults
from ...utils.config_utils import merge_cfg
from ...utils.computation_utils import compute_lame_parameters  
from ...utils.const import I_DTYPE,F_DTYPE,NORM_EPS
from ...utils.taichi_utils import qmul3d, qrot2d, qrot3d, w2quat2d, w2quat3d, inv_trans2d, inv_trans3d, cross2d
...
class PrimitiveBase:
    ...
    def __init__(self,solver:'MPMSolver',cfg:Union[BoxConfig,SphereConfig,MeshConfig,EllipsoidConfig]):
        ...
        cfg.particle_info.mu_0, cfg.particle_info.lambd_0 = compute_lame_parameters(
            cfg.particle_info.E_0, cfg.particle_info.nu_0
        ) # recompute lame parameter
        
        self.cfg = cfg
        
        # Get basic attribute
        self.solver = solver
        self.solver.particle_group_info[cfg.particle_id] = cfg.particle_info # append to particle group information in the solver
        ...
        # region Initialization for Rigid Body
        if self.is_rigid:
            ...
            ti.root.dense(ti.i, (solver.max_substeps,)).place(self.position, self.rotation, self.wrench, self.twist)
            if solver.needs_grad:
                ti.root.dense(ti.i, (solver.max_substeps,)).place(self.position.grad, self.rotation.grad,self.wrench.grad, self.twist.grad)
    ...
    def collide(self, dt, I, grid_v_I, grid_m_I, s, gravity):
        ...
        if grid_m_I > 0 and ((self.softness > 0 and influence > 0.1) or signed_dist <= 0.0): # Influence only works if it is above a threshold(for soft robot) or if it is inside of the body(signed_dist < 0)
    ...
    # region ToDo for implementation in children class
    @ti.kernel
